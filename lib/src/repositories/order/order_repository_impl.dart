...
class OrderRepositoryImpl implements OrderRepository {
  ...
  Future<void> changeStatus(int id, OrderStatus status) async {
    try {
      ...
      log('Erro ao atualizar status ${status.acronym}',
          error: e, stackTrace: s);
      throw RepositoryException(
          message: 'Erro ao atualizar status ${status.acronym}');
    }
  }
  ...
  Future<List<OrderModel>> findAllOrders(DateTime date,
    ...
    try {
      final orderResponse = await _dio.auth().get('/orders', queryParameters: {
